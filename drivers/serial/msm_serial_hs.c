...
#if 1 /* BT_001 Customize for Bluetooth [006] */
#include <linux/gpio.h>

/* BT_001 GPIO mapping for RTS CTS UART Tx/Rx */
#define UART_PORT_RTS           134     /* BSP_UART1DM_RFR_N */
#define UART_PORT_CTS           135     /* BSP_UART1DM_CTS_N */
...
/* BT_001 Port Configuration Table */
static unsigned uart_gpio_config[] = {
    /*        gpio,         func, dir,         pull,         drvstr */
#if defined(CONFIG_MACH_DECKARD_AF30)
...
#endif /* BT_001 Customize for Bluetooth [006] */
#if 1 /* BT_003 Customize for Bluetooth */
...
#if 1 /* BT_001 Customize for Bluetooth [010] */
...
#endif /* BT_001 Customize for Bluetooth [010] */
...
#else /* BT_001 Customize for Bluetooth [010] */
	wake_lock(&msm_uport->dma_wake_lock);
#endif /* BT_001 Customize for Bluetooth [010] */
...
#if 0 /* BT_001 Customize for Bluetooth [003] */
	wake_lock_timeout(&msm_uport->rx.wake_lock, HZ / 2);
#endif /* BT_001 Customize for Bluetooth [003] */
...
static int msm_hs_check_clock_off_locked(struct uart_port *uport)
{
	...
	if(gpio_get_value(UART_PORT_RTS) == MSM_HS_SERIAL_GPIO_LOW) {
		gpio_set_value( UART_PORT_RTS, MSM_HS_SERIAL_GPIO_HIGH );        /* BSP_UART1DM_RFR_N HIGH */
		/* BT_001 RTS Port Configuration */
...
#else /* BT_001 Customize for Bluetooth [010] */
#if 1 /* BT_001 Customize for Bluetooth [003] */
	wake_unlock(&msm_uport->rx.wake_lock);
#endif /* BT_001 Customize for Bluetooth [003] */
	wake_unlock(&msm_uport->dma_wake_lock);
#endif /* BT_001 Customize for Bluetooth [010] */
...
void msm_hs_request_clock_off(struct uart_port *uport) {
    ...
    if (clk_state == MSM_HS_CLK_ON) {
        ...
        spin_lock_irqsave(&uport->lock, flags);
        gpio_set_value( BT_WAKE_OUT, MSM_HS_SERIAL_GPIO_HIGH );          /* BT_WAKE SLEEP */
        spin_unlock_irqrestore(&uport->lock, flags);
...
#if 1 /* BT_001 Customize for Bluetooth [010] */
/* BT_001 request to turn off uart clock once pending TX is flushed */
void msm_hs_request_clock_off_force(struct uart_port *uport) {
    ...
    &&  (msm_uport->clk_state != MSM_HS_CLK_OFF)) {
        ...
        /* BT_001 we really want to clock off */
        clk_disable(msm_uport->clk);
        if (msm_uport->pclk)
            clk_disable(msm_uport->pclk);
        msm_uport->clk_state = MSM_HS_CLK_OFF;
...
#if 1 /* BT_002 Customize for Bluetooth */
/* BT_002 request to uart clock state */
...
static void msm_hs_request_clock_on_locked(struct uart_port *uport) {
	...
	case MSM_HS_CLK_REQUEST_OFF:
		...
		gpio_set_value( UART_PORT_RTS, MSM_HS_SERIAL_GPIO_LOW );		/* BSP_UART1DM_RFR_N LOW */
#endif /* BT_003 Customize for Bluetooth */
...
void msm_hs_request_clock_on(struct uart_port *uport) {
	...
	spin_lock_irqsave(&uport->lock, flags);
#if 1 /* BT_003 Customize for Bluetooth */
	gpio_set_value( BT_WAKE_OUT, MSM_HS_SERIAL_GPIO_LOW );          /* BT_WAKE WAKE */
#endif /* BT_003 Customize for Bluetooth */
...
static irqreturn_t msm_hs_wakeup_isr(int irq, void *dev)
{
	...
	/* HOST_WAKE FALLING */
	if(host_wake_state == MSM_HS_SERIAL_GPIO_LOW) {
		if(clk_state == MSM_HS_CLK_OFF) {
			spin_lock_irqsave(&uport->lock, flags);
	...
	if (msm_uport->clk_state == MSM_HS_CLK_OFF)  {
		/* ignore the first irq - it is a pending irq that occured
		 * before enable_irq() */
#if 0 /* BT_001 Customize for Bluetooth [002] */
		if (msm_uport->wakeup.ignore)
			msm_uport->wakeup.ignore = 0;
		else
			wakeup = 1;
#else /* BT_001 Customize for Bluetooth [002] */
		if (msm_uport->wakeup.ignore) {
			msm_uport->wakeup.ignore = 0;
			wakeup = 1;
        }
#endif /* BT_001 Customize for Bluetooth [002] */
	}
...
#if 0 /* BT_002 Customize for Bluetooth */
	ret = request_irq(uport->irq, msm_hs_isr, IRQF_TRIGGER_HIGH,
			  "msm_hs_uart", msm_uport);
	if (unlikely(ret))
		return ret;
...
#if 0 /* BT_001 Customize for Bluetooth [001] */
	struct msm_serial_hs_platform_data *pdata = pdev->dev.platform_data;
#endif /* BT_001 Customize for Bluetooth [001] */
...
#if 1 /* BT_001 Customize for Bluetooth [001] */
...
#endif  /* BT_002 Customize for Bluetooth */
#else /* BT_001 Customize for Bluetooth [001] */
	if (pdata == NULL)
