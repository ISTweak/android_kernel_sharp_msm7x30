...
#define USB_MSC_FUNC_IOC_GET_REVISION_ID _IOW(USB_MSC_IOC_MAGIC, 0x24, int)
		      ...
		      unsigned int cmd, unsigned long arg)
{
	...
	case USB_MSC_FUNC_IOC_GET_MOUNT_STS:
		ret = ctxt->mount_enb;
		break;
	/* inquiry custom vendor_id and product_id and revision get */
	...
	case USB_MSC_FUNC_IOC_GET_PRODUCT_ID:
		...
		break;
	case USB_MSC_FUNC_IOC_GET_REVISION_ID:
		if (!arg) {
			printk(KERN_ERR "%s:USB_MSC_FUNC_IOC_GET_REVISION_ID arg NULL err\n", __func__);
			ret = -EFAULT;
			break;
		}
		if (copy_to_user((void __user*)arg, USB_MSC_REVISION_ID, USB_MSC_REVISION_ID_LEN)) {
			printk(KERN_ERR "%s:USB_MSC_FUNC_IOC_GET_REVISION_ID copy_to_user err\n", __func__);
			ret = -EFAULT;
			break;
		}
		ret = USB_MSC_REVISION_ID_LEN;
		break;
	default:
		return -ENOTTY;
	}
...
static int msc_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct msc_context *ctxt = &_context;
