...
#if CONFIG_SH_USB_CUST
struct pm_qos_request_list *pm_qos_req_cpu;
struct pm_qos_request_list *pm_qos_req_sys;
#endif /* CONFIG_SH_USB_CUST */
...
#define USB_SUSPEND_PHY_TIMEOUT	HZ/2
...
#ifdef CONFIG_SH_USB_CUST
static int pid = USB_PID_MODE_CDC;  /* PENDIG */
#else /* CONFIG_SH_USB_CUST */
...
static unsigned long hs_connect_enable = 0; /* PENDIG */
static unsigned long user_pullup = 0;
static int reset_count = 0;

static int ldo_on;
static struct vreg *usb_pmic_vreg;
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
#define MODE_CDC	"MODE_CDC"
#define MODE_MTP	"MODE_MTP"
#define MODE_MSC	"MODE_MSC"
#define MODE_CDC_2	"MODE_CDC_2"
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
static inline void set_se1_gating(struct usb_info *ui)
{
	unsigned res = 0;

	if (!ui->pdata || ui->pdata->se1_gating == SE1_GATING_DEFAULT)
		return;

	res = ulpi_read(ui, ULPI_DIGOUT_CTRL);
	if (ui->pdata->se1_gating == SE1_GATING_ENABLE)
		res &=  ~ULPI_SE1_GATE;
	else
		res |=  ULPI_SE1_GATE;
	ulpi_write(ui, res, ULPI_DIGOUT_CTRL);
}
#endif /* CONFIG_SH_USB_CUST */
...
static void msm_hsusb_suspend_locks_acquire(struct usb_info *ui, int acquire)
{
	if (acquire) {
		wake_lock(&ui->wlock);
#if CONFIG_SH_USB_CUST
		pm_qos_update_request(pm_qos_req_cpu, ui->pdata->swfi_latency);
		/* targets like 7x30 have introduced core clock
		 * to remove the dependency on max axi frequency
		 */
		if (!ui->cclk)
			pm_qos_update_request(pm_qos_req_sys, MSM_AXI_MAX_FREQ);
#else /* CONFIG_SH_USB_CUST */
...
#if CONFIG_SH_USB_CUST
		pm_qos_update_request(pm_qos_req_cpu, PM_QOS_DEFAULT_VALUE);
		if (!ui->cclk)
			pm_qos_update_request(pm_qos_req_sys, PM_QOS_DEFAULT_VALUE);
#else /* CONFIG_SH_USB_CUST */
		pm_qos_update_requirement(PM_QOS_CPU_DMA_LATENCY,
					DRIVER_NAME,
...
static void msm_hsusb_suspend_locks_init(struct usb_info *ui, int init)
{
	if (init) {
		wake_lock_init(&ui->wlock, WAKE_LOCK_SUSPEND,
				"usb_bus_active");
#if CONFIG_SH_USB_CUST
		pm_qos_req_cpu = pm_qos_add_request(PM_QOS_CPU_DMA_LATENCY,
				PM_QOS_DEFAULT_VALUE);
		pm_qos_req_sys = pm_qos_add_request(PM_QOS_SYSTEM_BUS_FREQ,
				PM_QOS_DEFAULT_VALUE);
#else /* CONFIG_SH_USB_CUST */
		...
		pm_qos_remove_request(pm_qos_req_sys);
#else /* CONFIG_SH_USB_CUST */
...
static void handle_setup(struct usb_info *ui)
{
	...
	if( ( ( ctl.bRequestType & USB_TYPE_MASK) == USB_TYPE_VENDOR ) &&
		...
		if( pid == USB_PID_MODE_MTP && cnt_port <= 3 ) {
			switch( ctl.wIndex ){
...
static irqreturn_t usb_interrupt(int irq, void *data)
{
	...
	if (n & OTGSC_BSVIS) {
		...
		if (B_SESSION_VALID & n)	{
			...
			pr_info("usb cable disconnected\n");
...
#ifdef CONFIG_SH_USB_CUST
static irqreturn_t msm_hsusb_vbus_on_irq(int irq, void *data)
{
	struct usb_info *ui = the_usb_info;

	schedule_work(&ui->vbus_detect_work);
	return IRQ_HANDLED;
}

static void usb_vbus_detect(struct work_struct *w)
{
...
static int usb_pmic_enable_ldo(int enable)
{
	if (ldo_on == enable)
		return 0;
	...
	if (enable)
		return vreg_enable(usb_pmic_vreg);
	else
		return vreg_disable(usb_pmic_vreg);
}

static int usb_pmic_notify_init(void)
{
	usb_pmic_vreg = vreg_get(NULL, "usb");
	if (IS_ERR(usb_pmic_vreg)) {
		pr_err("%s: usb vreg get failed\n", __func__);
		vreg_put(usb_pmic_vreg);
		return PTR_ERR(usb_pmic_vreg);
	}

	return 0;
}

static void usb_pmic_notify_deinit(void)
{
	usb_pmic_enable_ldo(0);
	vreg_put(usb_pmic_vreg);
}
#endif /* CONFIG_SH_USB_CUST */
...
static void usb_prepare(struct usb_info *ui)
{
	...
	INIT_WORK(&ui->vbus_detect_work, usb_vbus_detect);
#endif /* CONFIG_SH_USB_CUST */
}
...
void usb_start(struct usb_info *ui)
{
	...
	if (ui->xceiv) {
...
#ifdef CONFIG_SH_USB_CUST
		int vbus_on_irq = ui->vbus_on_irq;

		if(!vbus_on_irq)
			goto out;

		ret = usb_pmic_notify_init();
		if (ret) {
			pr_err("%s: pm_app_rpc connect failed\n", __func__);
			goto out;
		}

		ret = request_irq(vbus_on_irq, msm_hsusb_vbus_on_irq,
				IRQF_TRIGGER_RISING, "msm_peripheral_vbus_on", NULL);
		if (ret) {
		...
		enable_irq_wake(vbus_on_irq);

		ret = usb_pmic_enable_ldo(1);
		if (ret) {
			pr_err("%s: unable to turn on internal LDO", \
					__func__);
			free_irq(vbus_on_irq, 0);
			usb_pmic_notify_deinit();
			goto out;
		}
		ui->vbus_sn_notif = 1;
...
static void usb_lpm_exit(struct usb_info *ui)
{
...
#ifdef CONFIG_SH_USB_CUST
	if (ui->usb_vreg && !IS_ERR(ui->usb_vreg))
		vreg_set_level(ui->usb_vreg, 3400);
#endif /* CONFIG_SH_USB_CUST */
	...
	ret = msm_xo_mode_vote(ui->xo_handle, MSM_XO_MODE_ON);
	if (ret)
...
static int usb_lpm_enter(struct usb_info *ui)
{
	...
	ret = msm_xo_mode_vote(ui->xo_handle, MSM_XO_MODE_OFF);
	if (ret)
...
#ifdef CONFIG_SH_USB_CUST
	if (ui->usb_vreg && !IS_ERR(ui->usb_vreg))
		vreg_set_level(ui->usb_vreg, 3075);
#endif /* CONFIG_SH_USB_CUST */
					 ...
					 char *buf)
{
	...
	unsigned long mode;
	int i; 
	char *mode_name[] = {MODE_CDC,MODE_MTP,MODE_MSC,MODE_CDC_2};
	...
	switch (pid) {
	case USB_PID_MODE_CDC:
		mode = 1;
		break;
	case USB_PID_MODE_MTP:
		mode = 2;
		break;
	case USB_PID_MODE_MSC:
		mode = 3;
		break;
	case USB_PID_MODE_CDC_2:
		mode = 4;
		break;
	default:
...
{
	unsigned long mode;
	unsigned long s_pid[4] = {USB_PID_MODE_CDC,USB_PID_MODE_MTP,USB_PID_MODE_MSC,USB_PID_MODE_CDC_2};
	struct usb_info *ui = the_usb_info;
	unsigned long functions_mask;
	unsigned short pid;
	int i;
	...
	if(((!strncmp(buf, "4", 1)) &&
	    ...
	    ((!strncmp(buf, MODE_CDC_2, strlen(MODE_CDC_2))) &&
		(size == strlen(MODE_CDC_2) || buf[strlen(MODE_CDC_2)] == 0x0a ||
		    buf[strlen(MODE_CDC_2)] == 0))) {
		mode = 4;
	}
	else if(((!strncmp(buf, "2", 1)) &&
		(size == 1 || buf[1] == 0x0a || buf[1] == 0)) ||
	    ((!strncmp(buf, MODE_MTP, strlen(MODE_MTP))) &&
		(size == strlen(MODE_MTP) || buf[strlen(MODE_MTP)] == 0x0a ||
		    buf[strlen(MODE_MTP)] == 0))) {
		mode = 2;
	}
	else if(((!strncmp(buf, "3", 1)) &&
		(size == 1 || buf[1] == 0x0a || buf[1] == 0)) ||
	    ((!strncmp(buf, MODE_MSC, strlen(MODE_MSC))) &&
		(size == strlen(MODE_MSC) || buf[strlen(MODE_MSC)] == 0x0a ||
		    buf[strlen(MODE_MSC)] == 0))) {
		mode = 3;
	}
	else if(((!strncmp(buf, "1", 1)) &&
		(size == 1 || buf[1] == 0x0a || buf[1] == 0)) ||
	    ((!strncmp(buf, MODE_CDC, strlen(MODE_CDC))) &&
		(size == strlen(MODE_CDC) || buf[strlen(MODE_CDC)] == 0x0a ||
		    buf[strlen(MODE_CDC)] == 0))) {
		mode = 1;
	}
	else {
...
#ifdef CONFIG_SH_USB_CUST
	ui->vbus_on_irq = vbus_on_irq;
	
	switch (msm_hsusb_get_boot_mode()) {
	case 1:
		pid = USB_PID_MODE_CDC_2;
		fs_connect_enable = 1;
		break;
	case 2:
		pid = USB_PID_MODE_CDC_2;
...
#ifdef CONFIG_SH_USB_CUST
	ui->xo_handle = msm_xo_get(MSM_XO_TCXO_D1, "usb");
	if (IS_ERR(ui->xo_handle)) {
		pr_err(" %s not able to get the handle"
			"to vote for TCXO D1 buffer\n", __func__);
		ret = PTR_ERR(ui->xo_handle);
		goto free_hs_cclk;
	}
...
#ifdef CONFIG_SH_USB_CUST
	ui->usb_vreg = vreg_get(NULL, "usb");
	if (IS_ERR(ui->usb_vreg)) {
		pr_err("%s: usb_vreg get failed\n", __func__);
		ui->usb_vreg = NULL;
		ret = PTR_ERR(ui->usb_vreg);
		goto free_vreg5v;
	}
	vreg_set_level(ui->usb_vreg, 3400);
#endif /* CONFIG_SH_USB_CUST */
