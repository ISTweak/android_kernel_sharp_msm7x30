...
#if 1  /* FEATURE_SHLOCAL_SHDRV */
#include <sharp/sh_oncrpc_id.h>
#endif /* FEATURE_SHLOCAL_SHDRV */
...
#if defined(CONFIG_USB_ANDROID_SH_CUST) || defined(CONFIG_SH_USB_CUST)
...
#endif	/* defined(CONFIG_USB_ANDROID_SH_CUST) || defined(CONFIG_SH_USB_CUST) */
};
...
#if defined(CONFIG_USB_ANDROID_SH_CUST) || defined(CONFIG_SH_USB_CUST)
		usb_rpc_ids.set_chgen			= 28;
		usb_rpc_ids.get_chgen			= 29;
		usb_rpc_ids.set_qxdmen			= 30;
		usb_rpc_ids.get_qxdmen			= 31;
		usb_rpc_ids.get_boot_mode		= 32;
#endif	/* defined(CONFIG_USB_ANDROID_SH_CUST) || defined(CONFIG_SH_USB_CUST) */
		return 0;
...
int msm_hsusb_set_qxdmen(uint32_t enable)
{
	...
	if (rc < 0) {
		pr_err("%s: set_chgen failed! rc = %d\n",
			__func__, rc);
		return rc;
	}
